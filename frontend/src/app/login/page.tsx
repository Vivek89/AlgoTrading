...
import { signIn, handleOAuthCallback } from '@/auth'
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        // Check if we have OAuth response from backend
      ...
      <div className="w-full max-w-md">
        <div className="card">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">AlgoTrading</h1>
            <p className="text-gray-600">Secure Trading Strategies</p>
          </div>
          ...
          {isLoading && (
            <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-blue-700 text-sm font-medium">Authenticating...</p>
            </div>
          )}
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-white border border-gray-300 text-gray-700 font-semibold py-2 px-4 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
